...
export default function CollectionAssignModal({ paper, onClose, onChanged, addToast }) {
  ...
  return <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/65 p-4" onMouseDown={onClose}>
    <section role="dialog" aria-modal="true" aria-label="Assign paper to collection" className="w-full max-w-lg rounded-xl border border-border bg-surface shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="p-5"><input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { event.stopPropagation(); if (event.key === 'Enter') { event.preventDefault(); assign(visible[selected]) } if (event.key === 'ArrowDown') setSelected((value) => Math.min(value + 1, visible.length - 1)); if (event.key === 'ArrowUp') setSelected((value) => Math.max(value - 1, 0)); if (event.key === 'Escape') { event.preventDefault(); onClose() } }} placeholder="Search collections" className="w-full rounded-md border border-border bg-background px-3 py-2.5 text-sm text-foreground outline-none focus:border-secondary" />
